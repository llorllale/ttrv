...
- Post an issue on the `tracker <https://github.com/tildeclub/tvr/issues>`_ describing the bug or feature you would like to add
...
1. Install the test dependencies
   ...
   .. code-block:: bash
      ...
      $ pip install tvr[test]

2. Set your ``$PYTHONPATH`` to point to the directory of your tvr repository.

    .. code-block:: bash

      $ export PYTHONPATH=~/code/tvr/

3. Run the tests using the existing cassettes
  
   .. code-block:: bash
    
     $ python -m pytest ~/code/tvr/tests/
     ================================ test session starts ================================
     platform linux -- Python 3.4.0, pytest-2.9.2, py-1.4.31, pluggy-0.3.1
     rootdir: ~/code/tvr/, inifile: 
...
4. By default, the cassettes will act as read-only.
   ...
   The easiest thing to do is to use the token generated by tvr when you log in.
   This is usually stored as *~/.local/share/tvr/refresh-token*.

   .. code-block:: bash
    
      $ python -m pytest ~/code/tvr/tests/ --record-mode once --refresh-token ~/.local/share/tvr/refresh-token
      ================================ test session starts ================================
      platform linux -- Python 3.4.0, pytest-2.9.2, py-1.4.31, pluggy-0.3.1
      rootdir: ~/code/tvr/, inifile: 
      plugins: xdist-1.14, cov-2.2.0
