...
TVR is not supported on Windows, due to a lack of resources and interest. Sorry!
...
Configuration files are stored in the ``{HOME}/.config/tvr/`` directory.

Check out [tvr.cfg](tvr/templates/tvr.cfg) for the full list of configurable options. You can clone this file into your home directory by running:
...
You can use [mailcap](https://en.wikipedia.org/wiki/Media_type#Mailcap) to configure how TVR will open different types of links.
...
This template contains examples for common MIME types that work with popular reddit websites like *imgur*, *youtube*, and *gfycat*. Open the mailcap template and follow the [instructions](tvr/templates/mailcap) listed inside.

Once you've setup your mailcap file, enable it by launching tvr with the ``tvr --enable-media`` flag (or set it in your **tvr.cfg**)

### Environment Variables

The default programs that TVR interacts with can be configured through environment variables:

<table>
  <tr>
  <td><strong>$TVR_EDITOR</strong></td>
  ...
  <tr>
  <td><strong>$TVR_BROWSER</strong></td>
  ...
  <tr>
  <td><strong>$TVR_URLVIEWER</strong></td>
...
TVR supports copying submission links to the OS clipboard. On macOS this is supported out of the box.
...
Themes can be used to customize the look and feel of TVR
...
You can list all installed themes with the ``--list-themes`` command, and select one with ``--theme``. You can save your choice permanently in your [tvr.cfg](tvr/templates/tvr.cfg) file. You can also use the <kbd>F2</kbd> & <kbd>F3</kbd> keys inside of TVR to cycle through all available themes.
...
<details>
 <summary>Why am I getting an error during installation/when launching tvr?</summary>
 
  > If your distro ships with an older version of python 2.7 or python-requests,
  > you may experience SSL errors or other package incompatibilities. The
  > easiest way to fix this is to install tvr using python 3. If you
  > don't already have pip3, see http://stackoverflow.com/a/6587528 for setup
  > instructions. Then do
  >
  > ```bash
  > $ sudo pip uninstall tvr
  > $ sudo pip3 install -U tvr
  > ```
...
<details>
  ...
  > unicode properly.
  >    
  > 1. Try starting TVR in ascii-only mode with ``tvr --ascii``
...
<details>
 <summary>How do I run the code directly from the repository?</summary>
  ...
  > assumes that you have cloned the repository into the directory **~/tvr_project**.
  >
  > ```bash
  > $ cd ~/tvr_project
  > $ python3 -m tvr
